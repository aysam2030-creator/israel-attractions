...
// ⚡ Bolt: Cache L.divIcon instances to prevent creating new objects on every render
function makePin(color: string, isTrip: boolean, step?: number) {
  const key = `${color}_${isTrip}_${step}`;
...
// ⚡ Bolt: Memoize Marker to prevent re-rendering all map markers on state change
const MapMarker = memo(({
  a, lang, isTrip, step, onPick, color
}: {
  a: Attraction, lang: Lang, isTrip: boolean, step?: number, onPick: (a: Attraction) => void, color: string
}) => (
  <Marker
