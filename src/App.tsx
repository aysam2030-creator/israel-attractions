...
// ⚡ Bolt Optimization: Cache Leaflet DivIcons to prevent recreating them on every render.
// Impact: Reduces GC pressure and memory usage significantly during map interactions.
...
// ⚡ Bolt Optimization: Memoize the Marker component to avoid unbinding/rebinding Leaflet events
// and recreating DOM elements on every state update (e.g., search input changes).
// Impact: Eliminates O(N) Leaflet DOM updates per React render, vastly improving main thread speed.
const MemoizedMarker = memo(({ a, lang, color, isTrip, step, setSelected }: { a: Attraction; lang: Lang; color: string; isTrip: boolean; step?: number; setSelected: (a: Attraction) => void }) => (
  <Marker
